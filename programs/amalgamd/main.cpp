...
int main(int argc, char** argv) {
   amalgam::plugin::initialize_plugin_factories();
   ...
   try {
      ...
      bpo::options_description app_options("Amalgam Daemon");
      bpo::options_description cfg_options("Amalgam Daemon");
      app_options.add_options()
            ("help,h", "Print this help message and exit.")
            ("data-dir,d", bpo::value<boost::filesystem::path>()->default_value("witness_node_data_dir"), "Directory containing databases, configuration file, etc.")
            ("version,v", "Print amalgamd version and exit.")
            ;

      bpo::variables_map options;

      for( const std::string& plugin_name : amalgam::plugin::get_available_plugins() )
         node->register_abstract_plugin( amalgam::plugin::create_plugin( plugin_name, node ) );
      ...
      {
         std::cout << "amalgam_blockchain_version: " << fc::string( AMALGAM_BLOCKCHAIN_VERSION ) << "\n";
         std::cout << "amalgam_git_revision:       " << fc::string( graphene::utilities::git_revision_sha ) << "\n";
