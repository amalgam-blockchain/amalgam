...
public:
   ...
   bool import_key(string wif_key)
   {
      ...
      amalgam::chain::public_key_type wif_pub_key = optional_private_key->get_public_key();
   ...
   int find_first_unused_derived_key_index(const fc::ecc::private_key& parent_key)
   {
      ...
      for (int key_index = 0; ; ++key_index)
      {
         ...
         amalgam::chain::public_key_type derived_public_key = derived_private_key.get_public_key();
   ...
   { try {
         ...
         amalgam::chain::public_key_type owner_pubkey = owner_privkey.get_public_key();
         amalgam::chain::public_key_type active_pubkey = active_privkey.get_public_key();
         amalgam::chain::public_key_type memo_pubkey = memo_privkey.get_public_key();
   ...
   {
      FC_ASSERT( tx_expiration_seconds < AMALGAM_MAX_TIME_UNTIL_EXPIRATION );
   ...
   std::map<string,std::function<string(fc::variant,const fc::variants&)>> get_result_formatters() const
   {
      ...
      m["list_my_accounts"] = [](variant result, const fc::variants& a ) {
         ...
         for( const auto& a : accounts ) {
            total_amalgam += a.balance;
            total_vest  += a.vesting_shares;
            total_abd  += a.abd_balance;
            out << std::left << std::setw( 17 ) << std::string(a.name)
                ...
                << std::right << std::setw(16) << fc::variant(a.abd_balance).as_string() <<"\n";
         }
         out << "-------------------------------------------------------------------------\n";
            out << std::left << std::setw( 17 ) << "TOTAL"
                << std::right << std::setw(18) << fc::variant(total_amalgam).as_string() <<" "
                << std::right << std::setw(26) << fc::variant(total_vest).as_string() <<" "
                << std::right << std::setw(16) << fc::variant(total_abd).as_string() <<"\n";
         return out.str();
      };
      ...
      m["get_order_book"] = []( variant result, const fc::variants& a ) {
         ...
         ss << ' ' << setw( ( spacing * 4 ) + 6 ) << "Bids" << "Asks\n"
            << ' '
            << setw( spacing + 3 ) << "Sum(ABD)"
            << setw( spacing + 1) << "ABD"
            << setw( spacing + 1 ) << "AMALGAM"
            << setw( spacing + 1 ) << "Price"
            << setw( spacing + 1 ) << "Price"
            << setw( spacing + 1 ) << "AMALGAM "
            << setw( spacing + 1 ) << "ABD " << "Sum(ABD)"
            << "\n====================================================================================================="
            << "|=====================================================================================================\n";
         ...
         for( size_t i = 0; i < orders.bids.size() || i < orders.asks.size(); i++ )
         {
            ...
            {
               bid_sum += asset( orders.bids[i].abd, ABD_SYMBOL );
               ss
   ...
   bool broadcast )
{
   ...
   op.abd_amount = abd_amount;
   op.amalgam_amount = amalgam_amount;
   op.fee = fee;
