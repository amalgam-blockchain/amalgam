...
uint64_t get_rshare_reward( const comment_reward_context& ctx )
{
   ...
   u256 rf(ctx.total_reward_fund_amalgam.amount.value);
   ...
   claim = ( claim * ctx.reward_weight ) / AMALGAM_100_PERCENT;

   u256 payout_u256 = ( rf * claim ) / total_claims;
   FC_ASSERT( payout_u256 <= u256( uint64_t( std::numeric_limits<int64_t>::max() ) ) );
   uint64_t payout = static_cast< uint64_t >( payout_u256 );

   if( is_comment_payout_dust( ctx.current_amalgam_price, payout ) )
      payout = 0;

   asset max_amalgam = to_amalgam( ctx.current_amalgam_price, ctx.max_abd );

   payout = std::min( payout, uint64_t( max_amalgam.amount.value ) );
