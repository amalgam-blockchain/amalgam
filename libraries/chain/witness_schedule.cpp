...
void reset_virtual_schedule_time( database& db )
{
   ...
   for( const auto& witness : idx )
   {
      db.modify( witness, [&]( witness_object& wobj )
      {
         ...
         wobj.virtual_scheduled_time = AMALGAM_VIRTUAL_SCHEDULE_LAP_LENGTH / (wobj.votes.value+1);
      } );
   }
}
...
void update_median_witness_props( database& db )
{
   ...
   /// sort them by abd_interest_rate
   std::sort( active.begin(), active.end(), [&]( const witness_object* a, const witness_object* b )
   {
      return a->props.abd_interest_rate < b->props.abd_interest_rate;
   } );
   uint16_t median_abd_interest_rate = active[active.size()/2]->props.abd_interest_rate;
   ...
   db.modify( wso, [&]( witness_schedule_object& _wso )
   {
      ...
      _wso.median_props.abd_interest_rate    = median_abd_interest_rate;
   } );
   ...
   db.modify( db.get_dynamic_global_properties(), [&]( dynamic_global_property_object& _dgpo )
   {
      ...
      _dgpo.abd_interest_rate  = median_abd_interest_rate;
   } );
}
...
{
   if( (db.head_block_num() % AMALGAM_MAX_WITNESSES) != 0 ) //wso.next_shuffle_block_num )
   {
      return;
   }
   ...
   active_witnesses.reserve( AMALGAM_MAX_WITNESSES );
   ...
   for( auto witness_count = selected_voted.size();
        sitr != schedule_idx.end() && witness_count < AMALGAM_MAX_WITNESSES;
        ++sitr )
   {
   ...
   {
      auto new_virtual_scheduled_time = new_virtual_time + AMALGAM_VIRTUAL_SCHEDULE_LAP_LENGTH / ((*itr)->votes.value+1);
   ...
   size_t expected_active_witnesses = std::min( size_t(AMALGAM_MAX_WITNESSES), widx.size() );
   FC_ASSERT( active_witnesses.size() == expected_active_witnesses, "number of active witnesses does not equal expected_active_witnesses=${expected_active_witnesses}",
                                       ("active_witnesses.size()",active_witnesses.size()) ("AMALGAM_MAX_WITNESSES",AMALGAM_MAX_WITNESSES) ("expected_active_witnesses", expected_active_witnesses) );
   ...
   db.modify( wso, [&]( witness_schedule_object& _wso )
   {
      ...
      for( size_t i = active_witnesses.size(); i < AMALGAM_MAX_WITNESSES; i++ )
      {
