...
template< typename OperationType=amalgam::protocol::operation >
class evaluator
{
   public:
      virtual ~evaluator() {}
...
template< typename EvaluatorType, typename OperationType=amalgam::protocol::operation >
...
#define AMALGAM_DEFINE_EVALUATOR( X ) \
class X ## _evaluator : public amalgam::chain::evaluator_impl< X ## _evaluator > \
{                                                                           \
   public:                                                                  \
      typedef X ## _operation operation_type;                               \
                                                                            \
      X ## _evaluator( database& db )                                       \
         : amalgam::chain::evaluator_impl< X ## _evaluator >( db )          \
      {}                                                                    \
                                                                            \
      void do_apply( const X ## _operation& o );                            \
};

#define AMALGAM_DEFINE_PLUGIN_EVALUATOR( PLUGIN, OPERATION, X )               \
class X ## _evaluator : public amalgam::chain::evaluator_impl< X ## _evaluator, OPERATION > \
{                                                                           \
   public:                                                                  \
      typedef X ## _operation operation_type;                               \
                                                                            \
      X ## _evaluator( amalgam::chain::database& db, PLUGIN* plugin )       \
         : amalgam::chain::evaluator_impl< X ## _evaluator, OPERATION >( db ), \
           _plugin( plugin )                                                \
      {}                                                                    \
                                                                            \
      void do_apply( const X ## _operation& o );                            \
