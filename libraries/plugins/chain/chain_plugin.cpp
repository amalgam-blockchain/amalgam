...
#include <amalgam/utilities/benchmark_dumper.hpp>
...
using fc::flat_map;
using amalgam::chain::block_id_type;
...
const amalgam::chain::database& chain_plugin::db() const { return my->db; }
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         ("shared-file-size", bpo::value<string>()->default_value("12G"), "Size of the shared memory file. Default: 12G. If running a full node, increase this value to 200G.")
...
void chain_plugin::plugin_startup()
{
   ...
   {
      auto statsd = appbase::app().find_plugin< amalgam::plugins::statsd::statsd_plugin >();
      if( statsd != nullptr )
      {
   ...
   amalgam::utilities::benchmark_dumper dumper;

   const auto& abstract_index_cntr = my->db.get_abstract_index_cntr();

   typedef amalgam::utilities::benchmark_dumper::index_memory_details_cntr_t index_memory_details_cntr_t;
   ...
   db_open_args.initial_supply = AMALGAM_INIT_SUPPLY;
      ...
      const chainbase::database::abstract_index_cntr_t& abstract_index_cntr )
   {
      ...
      {
         typedef amalgam::utilities::benchmark_dumper::database_object_sizeof_cntr_t database_object_sizeof_cntr_t;
      ...
      const amalgam::utilities::benchmark_dumper::measurement& measure =
   ...
   if(my->replay)
   {
      ...
      db_open_args.benchmark = amalgam::chain::database::TBenchmark(my->benchmark_interval, benchmark_lambda);
      last_block_number = my->db.reindex( db_open_args );

      if( my->benchmark_interval > 0 )
      {
         const amalgam::utilities::benchmark_dumper::measurement& total_data = dumper.dump(true, get_indexes_memory_details);
   ...
   {
      db_open_args.benchmark = amalgam::chain::database::TBenchmark(dump_memory_details, benchmark_lambda);
...
bool chain_plugin::accept_block( const amalgam::chain::signed_block& block, bool currently_syncing, uint32_t skip )
{
...
void chain_plugin::accept_transaction( const amalgam::chain::signed_transaction& trx )
{
   boost::promise< void > prom;
   write_context cxt;
   cxt.req_ptr = &trx;
...
amalgam::chain::signed_block chain_plugin::generate_block(
   const fc::time_point_sec when,
   const account_name_type& witness_owner,
   const fc::ecc::private_key& block_signing_private_key,
   uint32_t skip )
...
bool chain_plugin::block_is_on_preferred_chain(const amalgam::chain::block_id_type& block_id )
{
   ...
   return db().get_block_id_for_num( amalgam::chain::block_header::num_from_id( block_id ) ) == block_id;
}

void chain_plugin::check_time_in_block( const amalgam::chain::signed_block& block )
{
...
} } } // namespace amalgam::plugis::chain::chain_apis
