...
using amalgam::protocol::block_header;
using amalgam::protocol::signed_block_header;
...
public:
   ...
   virtual amalgam::protocol::chain_id_type get_chain_id() const override;
...
amalgam::protocol::chain_id_type p2p_plugin_impl::get_chain_id() const
{
...
void p2p_plugin::plugin_initialize(const boost::program_options::variables_map& options)
{
   ...
   my->user_agent = "Amalgam Reference Implementation";
...
void p2p_plugin::broadcast_block( const amalgam::protocol::signed_block& block )
{
   ulog("Broadcasting block #${n}", ("n", block.block_num()));
   my->node->broadcast( graphene::net::block_message( block ) );
}

void p2p_plugin::broadcast_transaction( const amalgam::protocol::signed_transaction& tx )
{
