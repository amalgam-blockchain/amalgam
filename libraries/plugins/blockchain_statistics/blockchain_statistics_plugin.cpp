...
struct operation_process
{
   ...
   void operator()( const transfer_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         if( op.amount.symbol == AMALGAM_SYMBOL )
            b.amalgam_transferred += op.amount.amount;
         else
            b.abd_transferred += op.amount.amount;
      });
   }
   ...
   void operator()( const interest_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         b.abd_paid_as_interest += op.interest.amount;
      });
   }
   ...
   void operator()( const transfer_to_vesting_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         b.amalgam_vested += op.amount.amount;
      });
   }
   ...
   void operator()( const fill_vesting_withdraw_operation& op )const
   {
      ...
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         ...
         if( op.deposited.symbol == AMALGAM_SYMBOL )
   ...
   void operator()( const convert_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         b.abd_conversion_requests_created++;
         b.abd_to_be_converted += op.amount.amount;
      });
   }
   ...
   void operator()( const fill_convert_request_operation& op )const
   {
      _db.modify( _bucket, [&]( bucket_object& b )
      {
         b.abd_conversion_requests_filled++;
         b.amalgam_converted += op.amount_out.amount;
      });
   }
};
...
void blockchain_statistics_plugin_impl::pre_operation( const operation_notification& o )
{
   ...
   for( auto bucket_id : _current_buckets )
   {
      if( o.op.which() == operation::tag< withdraw_vesting_operation >::value )
      {
         ...
         auto new_vesting_withdrawal_rate = op.vesting_shares.amount / AMALGAM_VESTING_WITHDRAW_INTERVALS;
...
AMALGAM_DEFINE_PLUGIN( blockchain_statistics, amalgam::blockchain_statistics::blockchain_statistics_plugin );
