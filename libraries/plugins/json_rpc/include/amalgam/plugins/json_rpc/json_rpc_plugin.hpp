...
#define AMALGAM_JSON_RPC_PLUGIN_NAME "json_rpc"

#define JSON_RPC_REGISTER_API( API_NAME )                                                       \
{                                                                                               \
   amalgam::plugins::json_rpc::detail::register_api_method_visitor vtor( API_NAME );              \
   for_each_api( vtor );                                                                        \
}
...
namespace amalgam { namespace plugins { namespace json_rpc {
...
class json_rpc_plugin : public appbase::plugin< json_rpc_plugin >
{
   public:
      ...
      static const std::string& name() { static std::string name = AMALGAM_JSON_RPC_PLUGIN_NAME; return name; }
   ...
   class register_api_method_visitor
   {
      public:
         register_api_method_visitor( const std::string& api_name )
            : _api_name( api_name ),
              _json_rpc_plugin( appbase::app().get_plugin< amalgam::plugins::json_rpc::json_rpc_plugin >() )
         {}
      ...
      private:
         ...
         amalgam::plugins::json_rpc::json_rpc_plugin& _json_rpc_plugin;
   };

}

} } } // amalgam::plugins::json_rpc

FC_REFLECT( amalgam::plugins::json_rpc::api_method_signature, (args)(ret) )
