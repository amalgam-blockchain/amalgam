#include <amalgam/plugins/market_history_api/market_history_api_plugin.hpp>
...
using namespace amalgam::plugins::market_history;

class market_history_api_impl
{
   public:
      market_history_api_impl() : _db( appbase::app().get_plugin< amalgam::plugins::chain::chain_plugin >().db() ) {}
...
DEFINE_API_IMPL( market_history_api_impl, get_ticker )
{
   ...
   {
      auto open = ASSET_TO_REAL( asset( itr->non_amalgam.open, ABD_SYMBOL ) ) / ASSET_TO_REAL( asset( itr->amalgam.open, AMALGAM_SYMBOL ) );
      itr = bucket_idx.lower_bound( boost::make_tuple( 0, _db.head_block_time() ) );
      result.latest = ASSET_TO_REAL( asset( itr->non_amalgam.close, ABD_SYMBOL ) ) / ASSET_TO_REAL( asset( itr->amalgam.close, AMALGAM_SYMBOL ) );
...
DEFINE_API_IMPL( market_history_api_impl, get_volume )
{
   ...
   do
   {
      result.amalgam_volume.amount += itr->amalgam.volume;
      result.abd_volume.amount += itr->non_amalgam.volume;
...
DEFINE_API_IMPL( market_history_api_impl, get_order_book )
{
   ...
   while( itr != order_idx.end() && itr->sell_price.base.symbol == ABD_SYMBOL && result.bids.size() < args.limit )
   {
      order cur;
   ...
   while( itr != order_idx.end() && itr->sell_price.base.symbol == AMALGAM_SYMBOL && result.asks.size() < args.limit )
   {
      order cur;
      cur.order_price = itr->sell_price;
...
{
   JSON_RPC_REGISTER_API( AMALGAM_MARKET_HISTORY_API_PLUGIN_NAME );
}
