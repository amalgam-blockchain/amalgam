#include <amalgam/market_history/market_history_api.hpp>

#include <amalgam/chain/amalgam_objects.hpp>

namespace amalgam { namespace market_history {
...
{
   public:
      market_history_api_impl( amalgam::app::application& _app )
         :app( _app ) {}
...
market_ticker market_history_api_impl::get_ticker() const
{
   ...
   {
      auto open = ( asset( itr->open_abd, ABD_SYMBOL ) / asset( itr->open_amalgam, AMALGAM_SYMBOL ) ).to_real();
      result.latest = ( asset( itr->close_abd, ABD_SYMBOL ) / asset( itr->close_amalgam, AMALGAM_SYMBOL ) ).to_real();
   ...
   result.amalgam_volume = volume.amalgam_volume;
   result.abd_volume = volume.abd_volume;
...
market_volume market_history_api_impl::get_volume() const
{
   ...
   do
   {
      result.amalgam_volume.amount += itr->amalgam_volume;
      result.abd_volume.amount += itr->abd_volume;
...
order_book market_history_api_impl::get_order_book( uint32_t limit ) const
{
   ...
   const auto& order_idx = app.chain_database()->get_index< amalgam::chain::limit_order_index >().indices().get< amalgam::chain::by_price >();
   auto itr = order_idx.lower_bound( price::max( ABD_SYMBOL, AMALGAM_SYMBOL ) );

   order_book result;

   while( itr != order_idx.end() && itr->sell_price.base.symbol == ABD_SYMBOL && result.bids.size() < limit )
   {
      order cur;
      cur.price = itr->sell_price.base.to_real() / itr->sell_price.quote.to_real();
      cur.amalgam = ( asset( itr->for_sale, ABD_SYMBOL ) * itr->sell_price ).amount;
      cur.abd = itr->for_sale;
      result.bids.push_back( cur );
      ++itr;
   }

   itr = order_idx.lower_bound( price::max( AMALGAM_SYMBOL, ABD_SYMBOL ) );

   while( itr != order_idx.end() && itr->sell_price.base.symbol == AMALGAM_SYMBOL && result.asks.size() < limit )
   {
      order cur;
      cur.price = itr->sell_price.quote.to_real() / itr->sell_price.base.to_real();
      cur.amalgam = itr->for_sale;
      cur.abd = ( asset( itr->for_sale, AMALGAM_SYMBOL ) * itr->sell_price ).amount;
...
market_history_api::market_history_api( const amalgam::app::api_context& ctx )
{
