...
class market_history_plugin : public amalgam::app::plugin
{
   public:
...
struct bucket_object : public object< bucket_object_type, bucket_object >
{
   ...
   uint32_t             seconds = 0;
   share_type           high_amalgam;
   share_type           high_abd;
   share_type           low_amalgam;
   share_type           low_abd;
   share_type           open_amalgam;
   share_type           open_abd;
   share_type           close_amalgam;
   share_type           close_abd;
   share_type           amalgam_volume;
   share_type           abd_volume;

   price high()const { return asset( high_abd, ABD_SYMBOL ) / asset( high_amalgam, AMALGAM_SYMBOL ); }
   price low()const { return asset( low_abd, ABD_SYMBOL ) / asset( low_amalgam, AMALGAM_SYMBOL ); }
};
...
FC_REFLECT( amalgam::market_history::bucket_object,
                     (id)
                     (open)(seconds)
                     (high_amalgam)(high_abd)
                     (low_amalgam)(low_abd)
                     (open_amalgam)(open_abd)
                     (close_amalgam)(close_abd)
                     (amalgam_volume)(abd_volume) )
CHAINBASE_SET_INDEX_TYPE( amalgam::market_history::bucket_object, amalgam::market_history::bucket_index )

FC_REFLECT( amalgam::market_history::order_history_object,
                     (id)
                     (time)
                     (op) )
CHAINBASE_SET_INDEX_TYPE( amalgam::market_history::order_history_object, amalgam::market_history::order_history_index )
