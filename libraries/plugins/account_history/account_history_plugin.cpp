#include <amalgam/plugins/account_history/account_history_plugin.hpp>
...
#define AMALGAM_NAMESPACE_PREFIX "amalgam::protocol::"

namespace amalgam { namespace plugins { namespace account_history {
...
void account_history_plugin::plugin_initialize( const boost::program_options::variables_map& options )
{
   ...
   AMALGAM_LOAD_VALUE_SET(options, "account-history-track-account-range", my->_tracked_accounts, pairstring);
   ...
   if( options.count( "track-account-range" ) )
   {
      ...
      AMALGAM_LOAD_VALUE_SET( options, "track-account-range", my->_tracked_accounts, pairstring );
   }
   ...
   if( options.count( "account-history-whitelist-ops" ) || options.count( "history-whitelist-ops" ) )
   {
      ...
      if( options.count( "account-history-whitelist-ops" ) )
      {
         for( auto& arg : options.at( "account-history-whitelist-ops" ).as< vector< string > >() )
         {
            ...
            {
               if( op.size() )
                  my->_op_list.insert( AMALGAM_NAMESPACE_PREFIX + op );
            }
         }
      }
...
} } } // amalgam::plugins::account_history
