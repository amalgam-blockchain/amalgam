...
{
   public:
      block_info_api( const amalgam::app::api_context& ctx );
...
FC_REFLECT( amalgam::plugin::block_info::get_block_info_args,
   (start_block_num)
   (count)
   )

FC_API( amalgam::plugin::block_info::block_info_api,
   (get_block_info)
