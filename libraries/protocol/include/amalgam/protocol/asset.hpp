...
namespace amalgam { namespace protocol {
   ...
   struct asset
   {
      asset( share_type a = 0, asset_symbol_type id = AMALGAM_SYMBOL )
      :amount(a),symbol(id){}
...
namespace fc {
    inline void to_variant( const amalgam::protocol::asset& var,  fc::variant& vo ) { vo = var.to_string(); }
    inline void from_variant( const fc::variant& var,  amalgam::protocol::asset& vo ) { vo = amalgam::protocol::asset::from_string( var.as_string() ); }
}

FC_REFLECT( amalgam::protocol::asset, (amount)(symbol) )
FC_REFLECT( amalgam::protocol::price, (base)(quote) )
