...
namespace amalgam { namespace protocol {
   ...
   {
      validate_account_name( author );
      FC_ASSERT( percent_amalgam_dollars <= AMALGAM_100_PERCENT, "Percent cannot exceed 100%" );
      FC_ASSERT( max_accepted_payout.symbol == ABD_SYMBOL, "Max accepted payout must be in ABD" );
   ...
   void equihash_pow::create( const block_id_type& recent_block, const account_name_type& account_name, uint32_t nonce )
   {
      ...
      proof = fc::equihash::proof::hash( AMALGAM_EQUIHASH_N, AMALGAM_EQUIHASH_K, seed );
   ...
   void equihash_pow::validate() const
   {
      validate_account_name( input.worker_account );
      auto seed = fc::sha256::hash( input );
      FC_ASSERT( proof.n == AMALGAM_EQUIHASH_N, "proof of work 'n' value is incorrect" );
      FC_ASSERT( proof.k == AMALGAM_EQUIHASH_K, "proof of work 'k' value is incorrect" );
   ...
   {
      validate_account_name( account );
      FC_ASSERT( is_asset_type( reward_amalgam, AMALGAM_SYMBOL ), "Reward Amalgam must be AMALGAM" );
      FC_ASSERT( is_asset_type( reward_abd, ABD_SYMBOL ), "Reward Amalgam must be ABD" );
      FC_ASSERT( is_asset_type( reward_vests, VESTS_SYMBOL ), "Reward Amalgam must be VESTS" );
      FC_ASSERT( reward_amalgam.amount >= 0, "Cannot claim a negative amount" );
      FC_ASSERT( reward_abd.amount >= 0, "Cannot claim a negative amount" );
      FC_ASSERT( reward_vests.amount >= 0, "Cannot claim a negative amount" );
      FC_ASSERT( reward_amalgam.amount > 0 || reward_abd.amount > 0 || reward_vests.amount > 0, "Must claim something." );
   }
