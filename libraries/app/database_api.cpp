...
class database_api_impl : public std::enable_shared_from_this<database_api_impl>
{
   public:
      ...
      std::shared_ptr< amalgam::follow::follow_api > _follow_api;
   ...
   : _db( *ctx.app.chain_database() )
{
   ...
   try
   {
      ...
      _follow_api = std::make_shared< amalgam::follow::follow_api >( ctx );
   }
...
vector<account_id_type> database_api_impl::get_account_references( account_id_type account_id )const
{
   ...
   const auto& refs = aidx.get_secondary_index<amalgam::chain::account_member_index>();
   ...
   return result;*/
   FC_ASSERT( false, "database_api::get_account_references --- Needs to be refactored for amalgam." );
}
...
void database_api::set_pending_payout( discussion& d )const
{
   ...
   if( itr != cidx.end() && itr->comment == d.id )  {
      d.promoted = asset( itr->promoted_balance, ABD_SYMBOL );
   }
                                                  ...
                                                  )const
{
   ...
   while( count > 0 && tidx_itr != tidx.end() )
   {
      ...
      try
      {
         ...
         result.back().promoted = asset(tidx_itr->promoted_balance, ABD_SYMBOL );
...
vector<discussion> database_api::get_discussions_by_promoted( const discussion_query& query )const
{
   return my->_db.with_read_lock( [&]()
   {
      ...
      auto tidx_itr = tidx.lower_bound( boost::make_tuple( tag, parent, share_type(AMALGAM_MAX_SHARE_SUPPLY) )  );
